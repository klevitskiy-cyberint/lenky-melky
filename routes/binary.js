...
var log = bunyan.createLogger({name: "binary"});
...
router.get('/', async function(req, res, next) {
  ...
  execCmdKasm(kasm.res.body.kasm_id, user_id, decodedUrl)
  const kasm_url = kasm.res.body.kasm_url;
  log.info(`kasm_url: ${kasm_url}`);

  res.send(`${process.env.KASM_BASE_URL}${kasm_url}?disable_tips=1&disable_chat=1&disable_fixed_res=1`);
});
 ...
 * @param {String} kasm_id kasm kasm id
 * @param {String} user_id kasm user id
 * @returns Object kasm
 */
async function execCmdKasm(kasm_id, user_id, decodedUrl) {
  const options = {
    url: `${process.env.KASM_BASE_URL}/api/public/exec_command_kasm`,
    method: 'POST',
    httpsAgent: agent,
    headers: {
        'Content-Type': 'application/json'
    ...
    data: {
        ...
        "user_id": user_id,
        "exec_config": {
            "cmd": `wget -O /home/kasm-user/Downloads/dangerous.file ${decodedUrl}`,
            "environment": {},
            "workdir": "/home/kasm-user/Downloads",
            "privileged": false,
            "user": "root"
        }
    },
